...
public:
    ...
    Node* helper(int* pre, int n, int &i, int &bound){
        
        if(i >= n || pre[i] >= bound)   return nullptr;
        
        Node* root = newNode(pre[i++]);
        
        root->left = helper(pre, n, i, root->data);
        
        root->right = helper(pre, n, i, bound);
    ...
    {
        //code here
        int bound = INT_MAX, i = 0;
        
        return helper(pre, size, i, bound);
    }
};
